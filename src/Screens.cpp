...
void HomeScreen::fillLine (int line, const DisplayDataProvider * dataProvider, char * dest)
{
	...
	if (line == 0)
	{
		sprintf(dest, "%c %02d.%02d.%04d %2s",
			(dataProvider->isDcfTimeAvailable()? CHAR_DCF : ' '),
			dayTime.tm_mday, 
			dayTime.tm_mon + 1, 
			dayTime.tm_year + 2000, 
...
void TimeSetting::fillLine (int line, const DisplayDataProvider * dataProvider, char * dest)
{
	if (line == 0)
	{
		sprintf(dest, "%c%02d.%02d.%04d  %2s", CHAR_SETTINGS, 
			...
			dataProvider->getDayTime().tm_year + 2000, 
...
AlarmSetting::Data EEMEM alarmDataEE1 = {true, 6, 50, {false, true, true, true, true, true, false}};
AlarmSetting::Data EEMEM alarmDataEE2 = {true, 7, 30, {false, true, true, true, true, true, false}};
AlarmSetting::Data EEMEM alarmDataEE3 = {false, 9, 10, {false, true, true, true, true, true, false}};
	...
	number(_number)
{
	...
	switch (number)
	{
		case 1:	eeprom_read_block(&data, &alarmDataEE1, sizeof(AlarmSetting::Data)); break;
		case 2:	eeprom_read_block(&data, &alarmDataEE2, sizeof(AlarmSetting::Data)); break;
		case 3:	eeprom_read_block(&data, &alarmDataEE3, sizeof(AlarmSetting::Data)); break;
	}
}
...
void AlarmSetting::modifyValue (int s)
{
	...
	switch (number)
	{
		case 1: eeprom_write_block(&data, &alarmDataEE1, sizeof(AlarmSetting::Data)); break;
		case 2: eeprom_write_block(&data, &alarmDataEE2, sizeof(AlarmSetting::Data)); break;
		case 3: eeprom_write_block(&data, &alarmDataEE3, sizeof(AlarmSetting::Data)); break;
	}
	sei();
}
