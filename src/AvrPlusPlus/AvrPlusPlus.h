...
public:
	...
	static inline void setVoltage (float _voltage) { voltage = _voltage; };
	static inline float getVoltage () { return voltage; };
};
...
public:
	 ...
	 */
	inline void setPullUp (bool pullUp) { putBit(pullUp); };
};
...
public:
	...
	inline int getErrorMs() const { return errorMs; };
};
...
public:
	 ...
	 */
	SpiDevice(Name spiPortName, unsigned char pinMosiNr, unsigned char pinSckNr, Name devicePortName, unsigned char pinCsNr);
	
	/** 
	 * @brief Procedure is called if the SPI device shall start the data reception.
	 */
	inline void startTransfer () { setLow(); };
	 ...
	 */
	inline void finishTransfer () { setHigh(); };
};
...
public:
	...
	AnalogComparator (IOPort::Name pinAin0Name, unsigned char pinAin0Nr, IOPort::Name pinAin1Name, unsigned char pinAin1Nr);
	int getValue () const;
...
public:
	...
	void stop ();
	inline void setChannelAValue(unsigned char val) { OCR0A = val; };
	inline void setChannelBValue(unsigned char val) { OCR0B = val; };
};
